...
function supabaseSettings(env) { return { url: env.SUPABASE_URL || '', publishableKey: env.SUPABASE_PUBLISHABLE_KEY || '', jwksUrl: env.SUPABASE_JWKS_URL || '' }; }
async function supabaseUser(request, env) { const settings = supabaseSettings(env); const authorization = request.headers.get('authorization') || ''; if (!settings.url || !settings.publishableKey || !authorization.startsWith('Bearer ')) return null; try { const response = await fetch(`${settings.url.replace(/\/$/, '')}/auth/v1/user`, { headers: { authorization, apikey: settings.publishableKey } }); if (!response.ok) return null; const user = await response.json(); return user?.id ? user : null; } catch { return null; } }
...
async function handle(request, env) {
  ...
  if (url.pathname === '/api/supabase/config' && method === 'GET') { const settings = supabaseSettings(env); return json({ success: true, configured: !!(settings.url && settings.publishableKey), url: settings.url, publishableKey: settings.publishableKey, jwksUrl: settings.jwksUrl, provider: 'supabase' }); }
  if (url.pathname === '/api/supabase/session' && method === 'GET') { const user = await supabaseUser(request, env); return json({ success: true, authenticated: !!user, user: user ? { id: user.id, email: user.email, name: user.user_metadata?.full_name || user.user_metadata?.name || '', avatar: user.user_metadata?.avatar_url || user.user_metadata?.picture || '' } : null }); }
  if (url.pathname === '/api/supabase/profile' && method === 'GET') { const user = await supabaseUser(request, env); if (!user) return json({ success: false, error: 'Supabase session is required' }, 401); const saved = env.APPS ? await env.APPS.get(`supabase-profile:${user.id}`, 'json') : null; return json({ success: true, profile: { id: user.id, email: user.email, name: user.user_metadata?.full_name || user.user_metadata?.name || '', avatar: user.user_metadata?.avatar_url || user.user_metadata?.picture || '', ...saved } }); }
  if (url.pathname === '/api/supabase/profile' && method === 'POST') { const user = await supabaseUser(request, env); if (!user) return json({ success: false, error: 'Supabase session is required' }, 401); if (!env.APPS) return json({ success: false, error: 'APPS KV binding is not configured' }, 503); let body; try { body = await request.json(); } catch { return json({ success: false, error: 'Invalid JSON' }, 400); } const profile = { name: safeText(body.name, 120), avatar: safeText(body.avatar, 2000), updated_at: new Date().toISOString() }; await env.APPS.put(`supabase-profile:${user.id}`, JSON.stringify(profile)); return json({ success: true, profile: { id: user.id, email: user.email, ...profile } }); }
